...
import { handleRt, handleAdminRt } from "../realtime";
...
export class ContactsController {
    ...
    public post(@Req() req: Request, @Res() res: Response): void {
        ...
        new Contact(req.body).save((error: any, response: any) => {
            if (error) {
                ...
                handleRt(response.userId, req, {type: DATA_CONTACTS_ADD, payload: {contact: response}});
                handleAdminRt(req, {type: DATA_CONTACTS_ADD, payload: {contact: response}});
                res.send(response);
            }
    ...
    public put(@Req() req: Request, @Res() res: Response): void {
        ...
        Contact.findOneAndUpdate({_id: new ObjectID(req.params.id)}, req.body, (error: any, response: any) => {
            if (response == null) {
                ...
                handleRt(response.userId, req, {type: DATA_CONTACTS_UPDATE, payload: {_id: response._id, contact: req.body}});
                handleAdminRt(req, {type: DATA_CONTACTS_UPDATE, payload: {_id: response._id, contact: req.body}});
                res.send(response);
                return;
            }
        });
    ...
    public delete(@Req()req: Request, @Res() res: Response): void {
        ...
        Contact.remove({_id: new ObjectID(req.params.id), userId: new ObjectID(userId)}, (error: any) => {
            ...
            handleRt(userId, req, {type: DATA_CONTACTS_REMOVE, payload: {_id: req.params.id}});
            handleAdminRt(req, {type: DATA_CONTACTS_REMOVE, payload: {_id: req.params.id}});
            res.sendStatus(200);
            }
        });
    }
