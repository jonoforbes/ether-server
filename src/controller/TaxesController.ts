...
import { Tax } from "../schema/TaxSchema";
import { User } from "../schema/UserSchema";
import { io, clientIdsMap } from "../index";
import { handleAuth, getToken } from "../auth";
var jwt: any = require("jsonwebtoken");

const DATA_TAXES_ADD: string = "DATA_TAXES_ADD";
const DATA_TAXES_REMOVE: string = "DATA_TAXES_REMOVE";
const DATA_TAXES_UPDATE: string ="DATA_TAXES_UPDATE";
const DATA_TAXES_ADD_ALL: string ="DATA_TAXES_ADD_ALL";

@JsonController("/api/taxes")
export class TaxesController {
    ...
    public get(@Req() req: Request, @Res() res: Response): void {
        ...
        Tax.find({userId: new ObjectID(userId)}, (error: any, taxes: any) => {
            if (error) {
                res.send(error);
                return;
            }
    ...
    public getAdmin(@Req() req: Request, @Res() res: Response): void {
        ...
        User.find({_id: new ObjectID(userId)}, (error: any, docs: any) => {
            ...
            if (docs[0].role !== "admin") {
                ...
                Tax.find({_id: {'$ne': null}}, (error: any, taxes: any) => {
                    if (error) {
                        res.send(error);
                        return;
                    }
    ...
    public getById(@Req() req: Request, @Res() res: Response): void {
        let userId: string = handleAuth(req, res);
        Tax.find({_id: new ObjectID(req.params.id), userId: new ObjectID(userId)}, (error: any, docs: any) => {
            if (error) {
                res.send(error);
                return;
            }
    ...
    public post(@Req() req: Request, @Res() res: Response): void {
        ...
        new Tax(req.body).save((error: any, response: any) => {
            if (error) {
                res.send(error);
                return;
            }
    ...
    public put(@Req() req: Request, @Res() res: Response): void {
        let userId: string = handleAuth(req, res);
        Tax.findOneAndUpdate({_id: new ObjectID(req.params.id), userId: new ObjectID(userId)}, req.body, (error: any, response: any) => {
            if (response == null) {
                this.post(req, res);
            }
            else {
    ...
    public delete(@Req()req: Request, @Res() res: Response): void {
        let userId: string = handleAuth(req, res);
        Tax.remove({_id: new ObjectID(req.params.id), userId: new ObjectID(userId)}, (error: any) => {
            if (error) {
                res.send(error);
                return;
            }
