...
const DATA_INCOMES_ADD: string = "DATA_INCOMES_ADD";
const DATA_INCOMES_REMOVE: string = "DATA_INCOMES_REMOVE";
const DATA_INCOMES_UPDATE: string ="DATA_INCOMES_UPDATE";
const DATA_INCOMES_ADD_ALL: string ="DATA_INCOMES_ADD_ALL";

@JsonController("/api/incomes")
export class IncomesController {
    ...
    public get(@Req() req: Request, @Res() res: Response): void {
        ...
        Income.find({userId: new ObjectID(userId)}, (error: any, incomes: any) => {
            if (error) {
                res.send(error);
                return;
            }
    ...
    public getAdmin(@Req() req: Request, @Res() res: Response): void {
        ...
        User.find({_id: new ObjectID(userId)}, (error: any, docs: any) => {
            ...
            if (docs[0].role !== "admin") {
                ...
                Income.find({_id: {'$ne': null}}, (error: any, incomes: any) => {
                    if (error) {
                        res.send(error);
                        return;
                    }
    ...
    public getById(@Req() req: Request, @Res() res: Response): void {
        let userId: string = handleAuth(req, res);
        Income.find({_id: new ObjectID(req.params.id), userId: new ObjectID(userId)}, (error: any, docs: any) => {
            if (error) {
                res.send(error);
                return;
            }
    ...
    public post(@Req() req: Request, @Res() res: Response): void {
        ...
        new Income(req.body).save((error: any, response: any) => {
            if (error) {
                res.send(error);
                return;
            }
    ...
    public put(@Req() req: Request, @Res() res: Response): void {
        let userId: string = handleAuth(req, res);
        Income.findOneAndUpdate({_id: new ObjectID(req.params.id), userId: new ObjectID(userId)}, req.body, (error: any, response: any) => {
            if (response == null) {
                this.post(req, res);
            }
            else {
    ...
    public delete(@Req()req: Request, @Res() res: Response): void {
        let userId: string = handleAuth(req, res);
        Income.remove({_id: new ObjectID(req.params.id), userId: new ObjectID(userId)}, (error: any) => {
            if (error) {
                res.send(error);
                return;
            }
